  ...
  getOrder() {
    ...
    this.api.getOrderById(this.id).then(data => {
      ...
      if (data) {
        ...
        this.connectAccountId = data.vid.connectedAcId;
  ...
  refundIt() {
    ...
    Swal.fire({
      ...
      if (data && data.value) {
        ...
        this.spinner.show();
        this.api
          .updateOrderStatus(this.id, "rejected")
          .then(() => {
            const is3d =
              this.payKey.startsWith("pi_") && !!this.connectAccountId;
            const param = is3d
              ? {
                  payment_intent: this.payKey,
                }
              : {
                  charge: this.payKey,
                };
            ...
            if (is3d)
              headers = headers.set("Stripe-Account", this.connectAccountId);
            this.api
              .httpPostWithHeaders("https://api.stripe.com/v1/refunds", param, {
                headers,
              })
              .subscribe(
                (data) => {
